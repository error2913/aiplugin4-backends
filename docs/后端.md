...
- Tools：`read_file`、`list_dir`、`write_file`（支持追加）、`delete_file`、`run_shell`、`export_file`
...
工具：`read_file` / `list_dir` / `write_file` / `delete_file` / `run_shell` / `export_file`。相对路径统一相对第一个沙箱根目录解析。
...
- 文件导出：`export_file` 只返回短时随机能力令牌下载地址，不暴露服务器绝对路径；导出路由不要求额外 Authorization（否则 QQ/Milky 协议端无法代取文件），但令牌有时效且每次请求重新校验沙箱路径与文件存在性；其它 MCP 请求仍受 token 鉴权保护
