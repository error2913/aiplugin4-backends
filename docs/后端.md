...
SealDice 核心正向 WS（`/core`，海豹主动连接）、OB11 协议端（`/onebot`）与 aiplugin4 插件（MCP `/mcp`）之间的透明中转：插件通过注入假消息执行核心/扩展指令并收集 bot 响应，支持拦截（不转发到协议端）或转发。
...
- 拓扑：`/core`（及 `/core/ws`）接海豹核心正向 WS，`/onebot` 接协议端；多消息捕获、lane 串行、拦截/转发、超时与断线处理内置
- 排障：中间件有核心/协议端连接与断开日志；`/healthz` 返回 `coreConnected` / `protocolConnected`；协议端未连接时核心发来的 API 请求（如 `get_login_info`）立即收到 `status: failed`，不会静默等待超时
