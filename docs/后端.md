...
| `ob11-core-bridge` | SealDice 核心/OB11 指令中转（MCP /control 兼容） | 46880 | Node |
...
SealDice 核心反向 WS（`/core`）、OB11 协议端（`/onebot`）与 aiplugin4 插件（MCP `/mcp`）之间的透明中转：插件通过注入假消息执行核心/扩展指令并收集 bot 响应，支持拦截（不转发到协议端）或转发。

- 默认端口 46880（Node，内置 http + ws）
- MCP（Streamable HTTP）：`http://127.0.0.1:46880/mcp`，工具 `run_ext_command` / `run_core_command`
- 兼容旧版 aiplugin4 控制 WS：`/control`（`command.invoke` 请求体与 `command.result` 响应保持一致）
- 拓扑：`/core` 接海豹核心反向 WS（或设 `AIPLUGIN4_BRIDGE_CORE_URL` 主动连接），`/onebot` 接协议端；多消息捕获、lane 串行、拦截/转发、超时与断线处理内置
